...
pub fn keyboard(_props: &KeyboardProps) -> Html {
    ...
    let allowed_range = state.legal_letters_for_index(state.solution.selected_index).unwrap_or_else(||Letter::Z..=Letter::A);
    let backspace_enabled = state.solution.letters[state.solution.selected_index as usize].is_some();
    html!(
        <div class="keyboard">
